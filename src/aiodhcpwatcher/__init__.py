...
class AIODHCPWatcher:
    ...
    async def async_start(self, if_indexes: Iterable[int] | None = None) -> None:
        ...
        socks: list[tuple[int, socket.socket, int]] = []
        future = self._loop.run_in_executor(None, self._start, socks, self._if_indexes)
        try:
        ...
        if len(self._socks) == 0:
            ...
            if self._socket_unavailable:
                ...
                # A dropped reader or a missing filter does not resolve itself,
                # so those are reported once rather than retried.
                self.restart_soon()
...
__all__ = ["DHCPRequest", "async_init", "async_start", "make_packet_handler"]
