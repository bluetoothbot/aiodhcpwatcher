...
async def test_execute_restart_preserves_if_indexes() -> None:
    ...
    def _capture(*args: object) -> None:
        # *args, not a spelled-out signature: _start's parameter list is owned by
        # the implementation, and a double that hardcodes its arity breaks the
        # moment that signature changes.
        if_indexes = args[-1] if args else None
        seen.append(
